...
// SAFETY: The created string will reference the underlying byte storage, so it must be kept alive
// as long as the returned GcBox.
pie_native_fn!(unsafe pie_string_new_static(ptr: *const u8, len: u64) -> GcBox {
    unsafe {
        let bytes = core::slice::from_raw_parts(ptr, len as usize);
        // SAFETY: The frontend must ensure the strings are UTF-8
